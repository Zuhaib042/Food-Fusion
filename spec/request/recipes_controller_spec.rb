...
RSpec.describe Food, type: :request do
    before(:each) do
        @user = User.new(name: 'Faranosh Amini', created_at: Time.now, updated_at: Time.now, email: 'faranosh.amini4@gmail.com', encrypted_password: '123456')
        @user.confirm
        sign_in @user
        @recipe = Recipe.new(name: 'pizza', preparation_time: '1 Hour', cooking_time: '2 hour', description: 'This is  the first recipe', public: true, created_at: Time.now, updated_at: Time.now, user_id: @user_id )      
